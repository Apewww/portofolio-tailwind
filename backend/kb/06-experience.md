# Professional Experience & Education

## Pengalaman Kerja

### Backend Engineer - RaflyLabs (2023 - Sekarang)
- Mengembangkan dan memelihara AI chatbot gateway dengan FastAPI dan Python (terhubung ke OpenRouter)
- Membangun sistem RAG untuk dokumentasi portofolio pribadi (ChromaDB + embedding model lokal)
- Mengembangkan platform streaming audio Syncra (backend API, admin panel, MCP server)
- Membangun MCP (Model Context Protocol) server untuk monitoring layanan
- Integrasi berbagai external API (OpenRouter, cuaca, audio streaming, YT-DLP)
- Deployment dan manajemen infrastruktur server (Windows Server, Nginx, reverse proxy)

## Pendidikan

### Universitas Jenderal Achmad Yani (Unjani) - Bandung (2023 - Sekarang)
- **Jurusan:** Teknik Informatika (S1)
- **Fokus:** Algoritma, sistem basis data, rekayasa perangkat lunak, arsitektur backend

### SMK Negeri 1 Cimahi (2019 - 2023)
- **Jurusan:** SIJA - Sistem Informatika Jaringan dan Aplikasi
- **Fokus:** Administrasi jaringan, pengembangan web, dan rekayasa perangkat lunak

## Sertifikasi

| Sertifikat | Penerbit | Link |
|---|---|---|
| HTML / CSS in Depth | Meta / Coursera | https://coursera.org/verify/UTJ9Y0Q04F0E |
| Introduction Frontend Systems | Meta / Coursera | https://coursera.org/verify/R3HXJABQVHH2 |
| Javascript Core Logic | Meta / Coursera | https://coursera.org/verify/Z654TDKKZJK2 |
| Python Engineering | Meta / Coursera | https://coursera.org/verify/6IIWE8U7861R |
| Version Control Workflow | Meta / Coursera | https://coursera.org/verify/PI22FH6PPXS7 |
